  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            >
              <div className={`p-2 rounded-xl transition-colors duration-300 ${isScrolled || location.pathname !== '/' ? 'bg-orange-100' : 'bg-white/20'
                }`}>
                <PawPrint className={`w-6 h-6 transition-colors duration-300 ${isScrolled || location.pathname !== '/' ? 'text-orange-500' : 'text-white'
                  }`} />
              </div>
              <span className={`text-xl font-bold transition-colors duration-300 ${isScrolled || location.pathname !== '/' ? 'text-gray-800' : 'text-white'
                }`}>
                宠物领养中心
              </span>
            </a>
            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => (
                <a
                  ...
                  className={`relative px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 hover:scale-105 ${isScrolled || location.pathname !== '/'
                    ? isActive(link.href)
              ...
              {isLoggedIn ? (
                ...
                <>
                  <Button
                    ...
                    className={`rounded-full ${isScrolled || location.pathname !== '/'
                  ...
                  <Button
                    onClick={() => navigate('/login')}
                    className={`rounded-full px-6 transition-all duration-300 ${isScrolled || location.pathname !== '/'
      ...
      >
        ...
        <div
          className={`absolute top-20 left-4 right-4 bg-white rounded-2xl shadow-warm-lg p-6 transition-all duration-300 ${isMobileMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0'
            }`}
        >
          <div className="flex flex-col gap-2">
            {navLinks.map((link) => (
              <a
                ...
                className={`px-4 py-3 rounded-xl font-medium transition-colors ${isActive(link.href)
                  ? 'text-orange-500 bg-orange-50'
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-8 right-8 z-50 w-12 h-12 rounded-full bg-orange-500 text-white shadow-warm-lg flex items-center justify-center transition-all duration-300 hover:bg-orange-600 hover:scale-110 ${showBackToTop
